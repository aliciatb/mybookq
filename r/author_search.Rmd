...
getData <- reactive({
  ...
  books <- books %>%
    ...
    group_by(title,name) %>%
    summarize(publishedDate = max(publishedDate),
...
```{r}
textAreaInput("author_query", label = "Favorite Authors:", value = favorite_authors, rows=3, resize = "vertical")
```
...
```{r upcoming_books_timeline, fig.height=4}
...
renderPlot({
  ...
  if(length(data_view) > 0){
    ...
    book_releases <- data_view %>%
      group_by(publishedDate) %>%
      summarize(count = n())
    
    typeof(Sys.Date())
    
    max_books <- max(book_releases$count)
    min_date <- as.Date(Sys.Date())
    max_date <- as.Date(max(book_releases$publishedDate))
    
    ggplot(book_releases, aes(publishedDate, count, group = 1)) +
      geom_point() +
      geom_line() +
      scale_y_continuous(limits = c(1,max_books)) +
      labs(x = "Date", y = "Books") 
