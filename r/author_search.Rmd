...
continuing_series_words <- "return,returns,returning,next,series,continue,continuing"
...
anticipationScores <- function(){
  ...
  continuing_series_words <- input$description_continuing_series
  ...
  positive_words <- paste0(positive_words,",",continuing_series_words)
...
renderGauge({
  ...
  if(length(data_view) > 0){
    ...
    gauge(books_score, min = -25, max = 25, gaugeSectors(
      success = c(10, 100), warning = c(0, 10), danger = c(-25, 0)
    ))
  }
})
```
...
```{r upcoming_books_cloud, fig.width=15, fig.height=10}
...
Sentiment Analysis & Anticipation Score
============================================================================
...
### Upcoming Book Scores

```{r upcoming_book_scores, fig.width=10, fig.height=10}
...
renderTable({
  ...
  if(length(data_view) > 0){
      ...
      book_description_scores <- bind_cols(book_description_sentiments, book_description_anticipation)
      book_description_scores %>%
        select(title, sentiment, anticipation)
  }
  else{
    paste0('No results found for ',input$author_query)
  }
...
```{r upcoming_book_descriptions_sentiments, fig.width=15, fig.height=10}
...
- [Project](https://github.com/aliciatb/mybookq/projects)
